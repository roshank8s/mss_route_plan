...
class Traktop(models.Model):
 ...
 ################################################################################
    ...
    # def fetch_vehicle_data(self):
    #     """
    #     Build a VROOM‐style list of vehicle dicts, including only nonzero fields:
    #       - cost_type & cost_value → fixed_cost / cost_per_hour / cost_per_distance
    #       - skills (string "1,2,5") → list of ints
    #       - type → string
    #       - working_hours_start/end → time_window: [start_ts, end_ts]
    #       - break_start/end → breaks: [{"id", "time_windows", "service"}]
    #       - speed_factor → numeric 1–5
    #       - max_tasks → int
    ...
    #     # Helper: accept float, int, str "HH:MM[:SS]" or dt.time → (h, m, s)
    ...
    #     for vehicle in vehicles:
    #         # ── Skip vehicles without valid coords ─────────
    #         try:
    #             lat = float(vehicle.latitude)
    #             lon = float(vehicle.longitude)
    #         except (TypeError, ValueError):
    #             _logger.error("Invalid location for %s: %r / %r", vehicle.name, vehicle.latitude, vehicle.longitude)
    #             continue
    ...
    #                     vdict["skills"] = skills
    #             except ValueError:
    #                 _logger.error("Invalid skills for %s: %r", vehicle.name, vehicle.skills)

    #         # ── Type ──────────────────────────────────────
    #         if vehicle.type and vehicle.type.strip():
    #             vdict["type"] = vehicle.type.strip().lower()

    #         # ── Working hours → time_window ──────────────
    ...
    #             vdict["time_window"] = [
    #                 int(user_tz.localize(local_start).astimezone(utc).timestamp()),
    #                 int(user_tz.localize(local_end).  astimezone(utc).timestamp())
    #             ]
    ...
    #                 "id": vehicle.id,
    #                 "time_windows": [[
    #                     int(user_tz.localize(bstart).astimezone(utc).timestamp()),
    #                     int(user_tz.localize(bend).  astimezone(utc).timestamp()),
    #                 ]],
    #                 "service": (be_h*3600 + be_m*60 + be_s) - (bs_h*3600 + bs_m*60 + bs_s),
    #             }]
    ...
    #             if sf: vdict["speed_factor"] = sf

    #         # ── max_tasks, max_travel_time, max_distance ──
